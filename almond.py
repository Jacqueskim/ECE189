...
class Almond(Dataset):
    ...
    def __create_info_df_csv(self):
        ...
        if os.path.exists(self.info_df_csv_file):
            ...
            #     zip(*self.info_df['img_name'].apply(lambda name: Almond.get_image_size(
            ...
            if not  os.path.exists(mat_file_path):
                print('currently the path does not exist')
                sys.exit()

            # digit_struct_data = Almond.read_digit_mat(os.path.join(mat_file_path))
    ...
    def __gen_datasets(self):
        ...
        for _, row in self.info_df.iterrows():
            ...
            increase = round(square_bb_width * Almond.expansion_ratio / 2.)
    ...
    def read_digit_mat(mat_file):
        ...
        for j in range(f['/digitStruct/bbox'].shape[0]):
            img_name = Almond.get_name(j, f)
            row_dict = Almond.get_bbox(j, f)
...
def Almond_get_datasets(data, load_train=True, load_test=True, resize_size=(96, 96), fold_ratio=2,
                      simplified=False):

    """ Returns Almond Dataset
    """
    (data_dir, args) = data
    print(data_dir)
    print(args)
...
def Almond_100_get_datasets(data, load_train=True, load_test=True):
    """ Returns Almond Dataset with 96x96 images
    """
    return Almond_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1)


def Almond_100_simplified_get_datasets(data, load_train=True, load_test=True):
    """ Returns Almond Dataset with 96x96 images and simplified labels: 1 for every digit
    """
    return Almond_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1,
                             simplified=True)
...
datasets = [
   {
       ...
       'input': (3, 74, 74),
       'output': (1, 2),
       'loader': Almond_100_get_datasets,
       'collate': Almond.collate_fn
   },
   {
       'name': 'Almond_100_simplified',
       'input': (3, 74, 74),
       'output': ([1]),
       'loader': Almond_100_simplified_get_datasets,
       'collate': Almond.collate_fn
   }
]
...
_, test_set = Almond_100_get_datasets((data_path, args), load_train=False, load_test=True)