...
while True:
	if ( comManager.find_sync() == 1 ):
		...
		if ( imageLen > 0 ):
			...
			imgConverter.convert(image, "Image_{}.png".format(count), w, h, pixelformat.decode('ASCII'))
			#hex_string = "".join("%02x" % b for b in image)
			#print (hex_string)
			#image_file = open("Image_{}.txt".format(count), "w")
			#image_file.write(hex_string)
			#image_file.close()
			
			

			image = cv2.imread("image_{}.png".format(count))
			cv2.imshow(" ", image)
			count = count + 1
			cv2.waitKey(1)
			#exit(0)